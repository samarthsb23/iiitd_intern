    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_Y_V_DATA_0: begin
                    rdata <= int_Y_V[31:0];
                end
                ADDR_Y_V_CTRL: begin
                    rdata[0] <= int_Y_V_ap_vld;
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (Y_V_ap_vld)
            int_Y_V_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_Y_V_CTRL)
            int_Y_V_ap_vld <= 1'b0; // clear on read
    end
end
