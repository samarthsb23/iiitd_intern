...
int main(int argc, char* argv[]) {
    ...
    float mat_type = 1;           // Specify to only run a single matrix type.
    ...
    for (unsigned int imat = 0; imat < NUM_MAT_TYPES; imat++) {
        ...
        if ((mat_type == 0 || imat + 1 == mat_type) && skip_mat_type - 1 != imat) {
            ...
            if (!(reduce_matrix_types && (imat == 2 || imat == 4 || imat == 7 || imat == 9))) {
                ...
                hls::stream<axis_out_t> matrixSStrm;
                hls::stream<axis_out_t> matrixUStrm;
                hls::stream<axis_out_t> matrixVStrm;

                for (int r = 0; r < N; r++) {
                    for (int c = 0; c < N; c++) {
                        axis_in_t temp;
                        temp.data = A[r][c];
                        temp.last = (r * N + c == N * N - 1); 
                        matrixAStrm.write(temp);
                    }
                }
                    ...
                    kernel_svd_new(matrixAStrm, matrixSStrm, matrixUStrm, matrixVStrm);

                    for (int r = 0; r < ROWS; r++) {
                        for (int c = 0; c < COLS; c++) {
                            axis_out_t s_elem = matrixSStrm.read();
                            S[r][c] = s_elem.data;
                        }
                    }
                    for (int r = 0; r < ROWS; r++) {
                        for (int c = 0; c < ROWS; c++) {
                            axis_out_t u_elem = matrixUStrm.read();
                            U[r][c] = u_elem.data;
                        }
                    }
                    for (int r = 0; r < COLS; r++) {
                        for (int c = 0; c < COLS; c++) {
                            axis_out_t v_elem = matrixVStrm.read();
                            V[r][c] = v_elem.data;
                        }
                    }
