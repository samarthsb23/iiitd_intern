    ...
    for (unsigned int imat = 0; imat < NUM_MAT_TYPES; imat++) {
        ...
        if ((mat_type == 0 || imat + 1 == mat_type) && skip_mat_type != imat + 1) {
            for (long unsigned i = 0; i < num_tests; i++) {
                ...
                std::string file_A =
                    base_path + "A_50_new.txt";
                std::string file_Q =
                    base_path + "Q_50_new.txt";
                std::string file_R =
                    base_path + "R_50_new.txt";
                ...
                for (int r = 0; r < A_ROWS; r++) {
                    for (int c = 0; c < A_COLS; c++) {
                        axis_in_t temp;
                        temp.data = A[r][c];
                        temp.last = (r * A_COLS + c == A_ROWS * A_COLS - 1); 
                        matrixAStrm.write(temp);
                    }
                }

                kernel_qrf_3_arch0(matrixAStrm, matrixQStrm, matrixRStrm);


                // Force zeros in lower triangle of our R, as we did that for LAPACK as well
                for (int r = 0; r < A_ROWS; r++) {
                    for (int c = 0; c < A_ROWS; c++) {
                        axis_out_t q_elem = matrixQStrm.read();
                        Q[r][c] = q_elem.data;
                        }
                    }
                    ...
                    for (int r = 0; r < A_ROWS; r++) {
                        for (int c = 0; c < A_COLS; c++) {
                            axis_out_t r_elem = matrixRStrm.read();
                            R[r][c] = (r > c) ? 0 : r_elem.data;
                        }
                    }
