...
__attribute__((sdx_kernel("matrixmul_3", 0))) void matrixmul_3(hls::stream<axis_data> &in_A, hls::stream<axis_data> &out_C);
...
#pragma HLS ARRAY_PARTITION variable=input_B complete dim=1
 ...
 loop_input_A1: for(row = 0; row<8; row++) {
  loop_input_A2: for(col = 0; col<8; col++) {
...
#pragma HLS LOOP_FLATTEN
 ...
 loop_input_B1: for(row = 0; row<8; row++) {
  loop_input_B2: for(col = 0; col<8; col++) {
#pragma HLS LOOP_FLATTEN
 ...
 loop1: for(row = 0; row < 8; row++) {
  loop2: for(col = 0; col<8; col++) {
   Mat_Dtype res = 0;
#pragma HLS PIPELINE
 loop3: for(index = 0; index < 8; index++) {
 ...
 loop_output_C1: for(row = 0; row < 8; row++) {
  loop_output_C2: for(col = 0; col < 8; col++){
#pragma HLS PIPELINE
