...
#ifndef _MUSIC_top_inMAT_re_H_
#define _MUSIC_top_inMAT_re_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "MUSIC_top_inMAT_re_memcore.h"

SC_MODULE(MUSIC_top_inMAT_re) {
...
protected:
    ...
    // buffer instances
    MUSIC_top_inMAT_re_memcore* buffer[BufferCount];
...
public:
    ~MUSIC_top_inMAT_re();
    SC_CTOR(MUSIC_top_inMAT_re) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "MUSIC_top_inMAT_re_memcore_U_";
            name += szTmp;

            buffer[i] = new MUSIC_top_inMAT_re_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "MUSIC_top_inMAT_re.trace");
            // ports
