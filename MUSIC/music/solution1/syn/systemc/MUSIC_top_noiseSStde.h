...
#ifndef _MUSIC_top_noiseSStde_H_
#define _MUSIC_top_noiseSStde_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "MUSIC_top_noiseSStde_memcore.h"

SC_MODULE(MUSIC_top_noiseSStde) {
...
public:
    ~MUSIC_top_noiseSStde();
    SC_CTOR(MUSIC_top_noiseSStde) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        MUSIC_top_noiseSStde_memcore_U = new MUSIC_top_noiseSStde_memcore("MUSIC_top_noiseSStde_memcore_U");
        MUSIC_top_noiseSStde_memcore_U->clk      ( clk );
        MUSIC_top_noiseSStde_memcore_U->reset    ( reset );
        MUSIC_top_noiseSStde_memcore_U->ce0      ( i_ce0 );
        MUSIC_top_noiseSStde_memcore_U->we0      ( i_we0 );
        MUSIC_top_noiseSStde_memcore_U->address0 ( memcore_iaddr );
        MUSIC_top_noiseSStde_memcore_U->d0       ( i_d0 );
        MUSIC_top_noiseSStde_memcore_U->q0       ( i_q0 );

        MUSIC_top_noiseSStde_memcore_U->ce1      ( t_ce0 );
        MUSIC_top_noiseSStde_memcore_U->we1      ( t_we0 );
        MUSIC_top_noiseSStde_memcore_U->address1 ( memcore_taddr );
        MUSIC_top_noiseSStde_memcore_U->d1       ( t_d0 );
        MUSIC_top_noiseSStde_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "MUSIC_top_noiseSStde.trace");
            // ports
